...
**A real-time weather web platform for Sicily, powered by an IoT weather station running on a Raspberry Pi.**
...
**Meteo Sicilia** is not "just a weather site": it is a complete chain that connects a **real sensor** to a **screen**. A weather station built around a Raspberry Pi measures environmental conditions, transmits them to a backend that stores them, and distributes them in real time to a modern, responsive web interface.

The project brings together three layers — the **hardware** (the station with its sensors), the **backend** (which receives and stores the data) and the **frontend** (the site that displays it) — supported by two intelligent sources: an **AI model** for local forecasting and **Open-Meteo** for island-wide coverage.

## ✨ Features

- 📡 **Real-time data** from the station's sensors, updated without reloading the page (Server-Sent Events).
- 🤖 **AI forecast** of the upcoming hours for the city hosting the station (Palermo).
- 🗺️ **Full Sicily coverage**: professional data and forecasts from Open-Meteo for every other town.
...
- 📱 **Responsive design** carefully tuned for desktop, tablet and mobile.
...
Palermo **does not work like the other towns**: as the station's home city, its current data comes from the **real sensors** and its short-term forecast from the **AI model**. This logic is consistent across the whole app (search, chart, details). Every other town uses Open-Meteo.
...
- **HTTPS/TLS** with a free certificate (Let's Encrypt).
- **Cloudflare Tunnel** to expose the home station without opening router ports or revealing the home IP.
- **WHOIS privacy** and registrar lock on the domain.
- **CORS** configured to authorize the frontend in a controlled way.
...
<div align="center">
<sub>Meteo Sicilia · from sensor to screen 🌤️</sub>
</div>
